# What the reading implies somebody should build

**Addressed to [ynoia](https://github.com/ajreynol/kanon/blob/main/tools/ynoia/README.md), the project that asks what this ecosystem is
missing.** Five recommendations and one observation, in priority order, each
produced by reading a source rather than by wanting something.

**The entries open with the fields a ynoia request opens with** — *What*,
*Where*, *State* — so that carrying one costs reading rather than rewriting.
**They are deliberately not complete `tools.md` listings:** `Settles`, `Today`
and `Why here` are that page's own judgements about its own ordering, and
filling them in from here would be this tree grading somebody else's register.

**This page cannot file anything in anybody's tree and does not try.** Nothing
here is a ticket, nothing here is approved, and a request reaches a member
through a person or not at all. What it produces is an argument with a
recommendation at the end, written so that agreeing or disagreeing costs a
minute.
...
**A wish with no reading behind it does not belong on this page.** It belongs on
[ynoia's own registers](https://github.com/ajreynol/kanon/blob/main/tools/ynoia/tools.md), which are better at it and have a standard for
it. **This page has no opinion about anything nobody read.**
...
day.** The fourth proposes a tool, which is the expensive kind of wrong, and its
own entry says the evidence for it is one instance.

## Position is the priority

**First is the one most worth starting**, as on ynoia's pages and for the same
reason: a page ordered by how much a thing would change puts the largest item
first every time and is useless to somebody with an afternoon. **Where cost and
value pull apart, the entry says so in words rather than hiding it in the
order.**

**A verdict of *not yet* stays on the page**, with what would change it. That is
the entry doing its job, not the entry failing.
...
fail. **Where:** anoieu, beside `tests/run.py`. **State:** open, and a request
rather than a tool.
...
tree for is dead, unreachable, or subsumed by another, and belongs in the
*not checked* list where it would at least be honest.
...
the register, the column and the expected count, recomputed on each run. **This
...
**Where:** anoieu, beside `scripts/doc_currency.py`. **State:** open, and a
request rather than a tool.
...
nothing granted it. It reports a trend.

*What it would show:* run against this page today, **it would not flatter
anybody here, and this document least of all.**
...
whether it is still open. **Where:** nobody's tree yet — **State:** a tool
rather than a request, and the argument for that is below.
...
**Why a tool and not a request.** A disagreement has two ends in two trees.
...
**Before it: a second instance, not a line of code.** A register with one entry
...
Free in the glossary as of 2026-09-17; **a name is not available because the
glossary is silent**, so check the trees before using it.

## Not yet — signing what a policy check concluded

**What:** a signed, machine-readable statement of which checker commit ran
against which tree commit with what verdict. **Where:** anoieu, alongside the
checker it would attest. **State: not yet**, and recording why is the useful
half.
...
**Why not yet.** Every tree has one owner, everything here is public, nobody
...
## One observation for `papers.md`, which is not a tool
